...
pub const APP_VERSION: &str = "12.0.0";
...
impl ProcessMode {
    ...
    pub fn needs_base(self) -> bool {
        !matches!(self, Self::DedupOnly)
    }

    pub fn needs_dedup(self) -> bool {
        !matches!(self, Self::NameMatchOnly)
    }
}
...
impl MatchLevel {
    ...
    pub fn number(self) -> u8 {
        match self {
            Self::L01 => 1, Self::L02 => 2, Self::L03 => 3, Self::L04 => 4, Self::L05 => 5,
            Self::L06 => 6, Self::L07 => 7, Self::L08 => 8, Self::L09 => 9, Self::L10 => 10,
            Self::L11 => 11, Self::L12 => 12, Self::L13 => 13, Self::L14 => 14, Self::L15 => 15,
            Self::L16 => 16, Self::L17 => 17,
        }
    }

    pub fn code(self) -> &'static str {
        match self {
            Self::L01 => "L1", Self::L02 => "L2", Self::L03 => "L3", Self::L04 => "L4", Self::L05 => "L5",
            Self::L06 => "L6", Self::L07 => "L7", Self::L08 => "L8", Self::L09 => "L9", Self::L10 => "L10",
            Self::L11 => "L11", Self::L12 => "L12", Self::L13 => "L13", Self::L14 => "L14", Self::L15 => "L15",
            Self::L16 => "L16", Self::L17 => "L17",
        }
    }

    pub fn exact_phase(self) -> bool {
        !matches!(self, Self::L05 | Self::L06 | Self::L07 | Self::L15)
    }

    pub fn fuzzy_phase(self) -> bool {
        !self.exact_phase()
    }
...
impl Default for FieldMap {
    fn default() -> Self {
        Self {
            ...
            middle_initial: "middle_initial".into(),
            last_name: "last_name".into(),
            birthdate: "birthdate".into(),
            barangay_code: "barangay_code".into(),
...
impl GpuMode {
    pub fn label(self) -> &'static str {
        match self {
            Self::Auto => "Auto — benchmark and use GPU when beneficial",
            Self::ForceGpu => "Prefer GPU — fall back to CPU on error",
...
pub struct PerformanceConfig {
    ...
    pub exact_batch_size: usize,
    pub fuzzy_source_chunk: usize,
    pub fuzzy_candidate_cap: usize,
    pub gpu_mode: GpuMode,
    pub gpu_min_field_pairs: usize,
}

impl Default for PerformanceConfig {
    fn default() -> Self {
        let cpus = num_cpus::get().max(2);
        let reserve = 2.min(cpus.saturating_sub(1));
        Self {
            cpu_workers: cpus.saturating_sub(reserve).max(1),
            cpu_reserve_cores: reserve,
            memory_limit_percent: 82.0,
            min_free_ram_gb: 8.0,
            exact_batch_size: 100_000,
            fuzzy_source_chunk: 2_500,
            fuzzy_candidate_cap: 100,
            gpu_mode: GpuMode::Auto,
            gpu_min_field_pairs: 20_000,
        }
    }
}
...
pub struct ThresholdConfig {
    ...
    pub l5_component_min: f32,
    pub fuzzy_middle_min_l5: f32,
    pub fuzzy_middle_min_l7: f32,
}
...
impl Default for ThresholdConfig {
    fn default() -> Self {
        Self {
            ...
            l15: 96.0,
            minimum_margin: 3.0,
            l5_component_min: 90.0,
            fuzzy_middle_min_l5: 90.0,
            fuzzy_middle_min_l7: 92.0,
        }
    }
}
...
impl Default for OutputConfig {
    fn default() -> Self {
        Self {
            table_name: "nm_matched_results".into(),
...
pub struct AppConfig {
    ...
    pub source1: DbConfig,
    pub source2: DbConfig,
    pub destination: DbConfig,
    pub source1_fields: FieldMap,
    pub source2_fields: FieldMap,
    pub selected_levels: Vec<MatchLevel>,
    pub thresholds: ThresholdConfig,
    pub performance: PerformanceConfig,
    pub output: OutputConfig,
    pub operator_name: String,
    pub rebuild_base_index: bool,
}

impl Default for AppConfig {
    fn default() -> Self {
        Self {
            mode: ProcessMode::NameMatchOnly,
            consolidation: ConsolidationMode::ExactOnly,
            source1: DbConfig::default(),
            source2: DbConfig::default(),
            destination: DbConfig { database: "name_matching".into(), table: String::new(), ..DbConfig::default() },
            source1_fields: FieldMap::default(),
            source2_fields: FieldMap::default(),
            selected_levels: MatchLevel::CORE.to_vec(),
            thresholds: ThresholdConfig::default(),
            performance: PerformanceConfig::default(),
            output: OutputConfig::default(),
            operator_name: String::new(),
            rebuild_base_index: false,
        }
    }
}

impl AppConfig {
    pub fn selected(&self, level: MatchLevel) -> bool {
        self.selected_levels.contains(&level)
    }

    pub fn set_level(&mut self, level: MatchLevel, enabled: bool) {
        if enabled && !self.selected(level) {
            self.selected_levels.push(level);
        } else if !enabled {
            self.selected_levels.retain(|x| *x != level);
        }
        self.selected_levels.sort_by_key(|x| x.number());
    }

    pub fn reset_core_levels(&mut self) {
        self.selected_levels = MatchLevel::CORE.to_vec();
    }

    pub fn exact_plan(&self) -> Vec<MatchLevel> {
        let mut out: Vec<_> = self.selected_levels.iter().copied().filter(|x| x.exact_phase()).collect();
        out.sort_by_key(|x| x.number());
        out
    }

    pub fn fuzzy_plan(&self) -> Vec<MatchLevel> {
        let mut out: Vec<_> = self.selected_levels.iter().copied().filter(|x| x.fuzzy_phase()).collect();
        out.sort_by_key(|x| x.number());
        out
    }
}
